...
public ResponseEntity<Integer> deleteMessageById(@PathVariable Integer messageId) {
    // Call service method to delete the message by ID
    int rowsDeleted = messageService.deleteMessageById(messageId);
    
    // If no rows were deleted, return 200 OK with no content (0)
    if (rowsDeleted == 0) {
        return ResponseEntity.ok().build(); // Return 0 indicating no message was deleted
    }
    
    // If a row was deleted, return 200 OK with content (1)
    return ResponseEntity.ok(1); // Return 1 indicating the message was deleted
}
